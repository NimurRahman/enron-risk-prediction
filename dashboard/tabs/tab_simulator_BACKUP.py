...
Complete with individual scenarios, mass simulation, and model comparison
"""
...
def render(data: dict, filters: dict):
    ...
    # Merge predictions with features for complete data
    ...
    if not available_features:
        st.error("No matching features found between model and data")
        st.write("Model expects:", feature_names[:10])
        st.write("Data has:", list(full_data.columns[:10]))
        return
...
def render_feature_slider(feat: str, person_row: pd.Series, modified_values: dict):
    ...
    if abs(new_val - original_val) > step * 0.1:
        ...
        if change_pct > 0:
            st.caption(f"Up +{change_pct:.1f}% from original")
        else:
            st.caption(f"Down {change_pct:.1f}% from original")
...
def display_prediction_results(original_features, modified_features, all_predictions, engine, original_risk):
    ...
    try:
        ...
        if shap_values is not None:
            # Get feature names
            feature_names = list(modified_features.index)
...
def render_comparison_view(full_data, engine, available_features):
    ...
    if st.button("Compare Scenarios"):
        ...
        comparison_data = []
        for feat in available_features[:15]:  # Limit to top 15
